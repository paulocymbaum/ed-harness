...
Welcome to the **Asynchronous JavaScript** mock test.

## What is assessed

This screening covers module **03 — Asynchronous JavaScript**:

- Single-threaded execution, the call stack, and the event loop
- Microtask queue (promises) vs macrotask/callback queue (`setTimeout`)
- Promises, `async`/`await`, and how `await` schedules continuations
- `try`/`catch` error handling around asynchronous code
...
1. **Multiple choice** — 7 questions on the event loop, microtasks vs macrotasks, promises, and async/await
2. **Coding challenge** — implement a scheduling order labeler that reads sync/micro/macro tokens from stdin
