...
export function buildGenerateLessonPrompt(ctx: ContentMapSkillPromptContext): string {
  return [
    ...
    "",
    "Steps:",
    `1. Find the topic in the graph: node .cursor/tools/graph/find-node-by-index.js --course ${ctx.courseSlug} "${ctx.graphIndex}"`,
    `2. Scaffold the lesson folder: node scripts/graph/scaffold-from-graph.mjs --course ${ctx.courseSlug} "${ctx.graphIndex}"`,
