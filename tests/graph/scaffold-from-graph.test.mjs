...
test("requires --course", () => {
  try {
    execFileSync("node", [scaffoldScript, "01.8.1", "--dry-run"], {
      cwd: repoRoot,
      encoding: "utf8",
    });
...
test("dry-run returns planned paths without creating files", () => {
  const out = execFileSync(
    "node",
    [scaffoldScript, "--course", "javascript", "01.8.1", "--dry-run"],
    {
      cwd: repoRoot,
      encoding: "utf8",
    },
  ...
  assert.equal(parsed.courseSlug, "javascript");
...
test("scaffold creates lesson.meta.json with correct graphIndex", () => {
  ...
  try {
    ...
    const out = execFileSync("node", [scaffoldScript, "--course", "javascript", "01.8.1"], {
      cwd: repoRoot,
      encoding: "utf8",
    });
    const parsed = JSON.parse(out);
...
test("scaffold is idempotent for README", () => {
  const readmePath = path.join(
    repoRoot,
    "course/javascript/modules/01-javascript-fundamentals/lessons/01.8.2-strict-equality/README.md",
  );
  const original = readFileSync(readmePath, "utf8");
  writeFileSync(readmePath, "# Custom preserved content\n", "utf8");
  try {
    execFileSync("node", [scaffoldScript, "--course", "javascript", "01.8.2"], { cwd: repoRoot });
